...
const DecisionModal =  ({ isShow, closeModal, modalTitle, actionText, actionType, data, dataType, todo }) => {
    ...
    const submit = async (e) => {
        ...
        if(dataType === 'del-todo' && data !== null){
            setLoading(true);
            deleteTodo();
        }
    };
    ...
    const deleteTodo = async () => {
        ...
        await Axios.delete(`${process.env.REACT_APP_TODO_URL}/todos/${data._id}`, storage.getConfigWithBearer())

        .then((resp) => {

            if(resp.data.status === 200 && resp.data.error === false){
                setStep(1);
                setMessage({ ...messageData, type: 'success', title: 'successful', message: 'Todo list deleted successfully' });
                setLoading(false);
                history.push('/dashboard/todo-list')
